...
class Compose(object):
    ...
    def __call__(self, image, prob_ins,label, ins_label):
        for t in self.segtransforms:
            image, prob_ins,label, ins_label = t(image, prob_ins,label, ins_label)
        return image, prob_ins,label, ins_label
...
class ToTensor(object):
    ...
    def __call__(self,image, prob_ins,label, ins_label):
        ...
        prob_ins = torch.from_numpy(prob_ins)
        ins_label = torch.from_numpy(ins_label)
        #image, prob_ins,label, ins_label
        if not isinstance(label, torch.LongTensor):
            label = label.long()
            ins_label = ins_label.long()
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self,image, prob_ins, label, ins_label):
        if random.random() < self.p:
            ...
            prob_ins = cv2.flip(prob_ins, 1)
            ins_label = cv2.flip(ins_label, 1)
