...
if not os.path.exists(sys.argv[3] + 'Annotations'):
    os.mkdir(sys.argv[3] + 'Annotations')
    ...
    for (begin_label, end_label) in gt_labels_tuple:
        ...
        for i in range(1, instance_num + 1):
            if i in np.unique(label_0) and i not in search_instance:
                ...
                if not os.path.exists(sys.argv[3]+'person_json/' + video_dir + '_%d.json' % i):
                    print('this is not person')
                    search_instance.append(i)
                    continue
                sort_all_mask = json.loads(open(sys.argv[3]+'person_json/' +  video_dir + '_%d.json' % i).read())
                for gallery_img, (x1, y1, x2, y2, mask) in sort_all_mask:
                    ...
                    if i in np.unique(prob):
                        continue
                    #image_patch = cv2.imread(gallery_img)
